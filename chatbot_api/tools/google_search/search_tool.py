...
def search_google(query: str) -> str:
    ...
    relevant_docs_list = retriever.retrieve_embeddings(contents, serper_response['links'], query)
...
def format_contents(contents: List[Document]) -> str:
    ...
    for i, content in enumerate(contents):
        formatted_string += f"Paragraph from link {content.metadata['url']}: [{content.page_content}]"
